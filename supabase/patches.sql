...
-- =====================================================================
-- ส่วน E: Auto-sync schedule.status ให้ตรงกับ audit จริง (แก้ยอด เสร็จ/ค้าง ไม่ตรง)
--   ปัญหา: ตอน submit ไคลเอนต์เรียก completeSchedule แบบ "พลาดได้ไม่ล้ม flow"
--          → บางงานตรวจจริงแล้วแต่ schedule ยังค้าง pending → นับ เสร็จ/ค้าง เพี้ยน
--   แก้: ผูก trigger ที่ DB — เมื่อ audit_headers ได้คะแนน (status != pending)
--        ให้ mark pending schedule ของ (โรงงาน+พื้นที่+ผู้ตรวจคนนั้น) เป็น completed อัตโนมัติ
--        ไคลเอนต์พลาดก็ไม่หลุด, mytasks / assign อ่าน schedule.status ตรงกันเสมอ
--   รันซ้ำปลอดภัย (create or replace + drop trigger if exists + backfill เฉพาะที่ยังค้าง)
-- =====================================================================

-- E1) ฟังก์ชัน sync: จับคู่ audit → pending schedule ที่ใกล้เคียงสุด แล้ว mark completed
create or replace function public.sync_schedule_on_audit()
returns trigger
language plpgsql security definer set search_path = public
as $$
begin
  -- ทำเฉพาะเมื่อ audit มีคะแนนจริงแล้ว (ไม่ใช่ pending / N-A ทั้งหมด)
    ...
    update public.schedules s
     ...
     where s.schedule_id = (
       select s2.schedule_id
        ...
        where s2.plant_id = new.plant_id
          and s2.area_id  = new.area_id
          and s2.status   = 'pending'
          and new.auditor_id = any(s2.auditor_ids)
        -- เลือก schedule ที่วันครบกำหนดใกล้วันตรวจที่สุด (กันกรณีมีหลายรอบ)
        order by abs(s2.audit_date - new.audit_date), s2.audit_date desc
        limit 1
     );
  end if;
  return null;
...
-- E2) ผูก trigger: ทำงานทั้งตอน insert และตอน status ถูกอัปเดต (recalc_audit_header จะดัน pending→good)
drop trigger if exists trg_sync_schedule on public.audit_headers;
create trigger trg_sync_schedule
  after insert or update of status on public.audit_headers
  for each row execute function public.sync_schedule_on_audit();

-- E3) Backfill ครั้งเดียว: ปิดงานที่ค้าง pending ทั้งที่มี audit จริงของผู้ตรวจที่ได้รับมอบหมายแล้ว
update public.schedules s
   set status = 'completed'
 where s.status = 'pending'
   and exists (
     select 1 from public.audit_headers h
      where h.plant_id = s.plant_id
        and h.area_id  = s.area_id
        and h.status  <> 'pending'
        and h.auditor_id = any(s.auditor_ids)
   );
