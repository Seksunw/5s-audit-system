...
- [~] **ย้ายอีเมลผู้พัฒนา (คุณเสกสรรค์) จาก `@pronovalabs.com` → `seksun_wongyang@suntorywellness.co.th`** — **domain ยืนยันแล้ว** คือ `suntorywellness.co.th` (ไม่ใช่ `nbd.co.th` ที่คาดไว้ก่อนหน้า — ตรงกับที่เห็น tenant "Suntory Wellness" ใน Power Platform admin center)
  1. **GitHub** — ✅ **เปลี่ยนแล้ว** (ผู้ใช้ทำเอง 2026-09-03) · verify แล้วว่า push/pull ยังทำงานปกติ (credential ผูกกับ account ไม่ใช่อีเมล) · `git config user.email` ในเครื่องนี้ **อัปเดตให้ตรงแล้วทั้ง global และ local scope** (เจอว่ามี local override ทับ global อยู่ก่อน แก้ครบทั้งคู่)
  2. **Claude** — ยังไม่ได้คุยว่าทำหรือยัง (นอกขอบเขตที่ทำแทนได้ ต้องทำเองผ่าน claude.ai account settings)
  3. **Supabase — 2 จุดแยกกัน:**
     - บัญชี Dashboard (เจ้าของ project) — ✅ **เปลี่ยนแล้ว** (ผู้ใช้ทำเอง 2026-09-03) · verify แล้วว่า MCP (`SUPABASE_ACCESS_TOKEN`) ยังเชื่อมต่อได้ปกติ
     - **ตัวเองในฐานะ admin ของแอป** (`profiles`/`auth.users` row ที่ยังเป็น `seksun@pronovalabs.com`) — **ยังไม่ทำ ผู้ใช้ขอ "รอก่อน" (2026-09-03)** ต้องแก้คู่กันทีหลัง: `auth.users.email` (ผ่าน Supabase Dashboard → Authentication → Users) + `profiles.email` (ผ่านหน้า Users ในแอป)
